...
#### Tests (RED)
- [x] Create `tests/test_tools.py`
- [x] Build `make_context()` fixture that creates a `ProcurementContext` with:
...
- [x] Build helper to invoke tools from the context (call `build_tools(ctx)`, find tool by name, invoke)
...
#### Tests (RED)
- [x] `test_place_order_on_time` — delivery before deadline → no warning in response
- [x] `test_place_order_late` — delivery after deadline → response contains "LATE" warning with days late and the deadline date
- [x] `test_place_order_no_gap_row` — component not in gap_df (edge case) → no crash, no warning
- [x] Run tests — verify RED

#### Implementation (GREEN)
- [x] In `place_order`, after computing `delivery_date`, look up `earliest_needed_by` from `ctx.gap_df` for the component
- [x] If `delivery_date > earliest_needed_by`, append a warning line to the return string:
  `⚠ LATE: delivers {delivery_date} but needed by {earliest_needed_by} ({N} days late)`
- [x] Also auto-create an alert: `ctx.placed_alerts.append(f"DEADLINE RISK: ...")`
...
#### Tests (RED)
- [x] `test_place_order_concentration_under_limit` — 40% after order → order goes through
- [x] `test_place_order_concentration_over_limit` — would be 65% after order (limit 50%) → ERROR response, order NOT placed
- [x] `test_place_order_concentration_error_includes_max_qty` — error message includes the max quantity this supplier can receive
- [x] `test_place_order_concentration_no_constraint` — component has no `CONCENTRATION_LIMIT` → no check, order goes through
- [x] `test_place_order_concentration_accounts_for_prior_orders` — two sequential orders to same supplier, second should count the first
- [x] Run tests — verify RED

#### Implementation (GREEN)
- [x] Add helper `_concentration_limit(ctx, component_id) -> dict | None` — returns `{"max_pct": 0.5, "secondary_min_pct": 0.2}` if a `CONCENTRATION_LIMIT` constraint applies to this component, else None
- [x] In `place_order`, after supplier validation but before recording:
...
#### Tests (RED)
- [x] `test_place_order_no_duplicate` — first order for this combo → goes through
- [x] `test_place_order_exact_duplicate` — same component, supplier, quantity already placed → ERROR, order NOT placed
- [x] `test_place_order_same_component_different_supplier` — different supplier → goes through (not a duplicate)
- [x] `test_place_order_same_component_different_quantity` — same supplier, different quantity → goes through (deliberate split)
- [x] Run tests — verify RED

#### Implementation (GREEN)
- [x] In `place_order`, before recording the order, check `ctx.placed_orders` for any entry matching all three of (component_id, supplier_id, quantity)
- [x] If found, return: `"ERROR: Duplicate order. {po_number} already placed {component_id} × {quantity} from {supplier_id}. Adjust quantity or choose a different supplier."`
- [x] Run tests — verify GREEN
...
#### Tests (RED)
- [x] `test_eligible_suppliers_shows_delivery_status` — output contains "ON TIME" or "LATE by X days" per supplier
- [x] `test_eligible_suppliers_shows_concentration` — output contains current concentration % and what it would be
- [x] `test_eligible_suppliers_shows_constraint_notes` — output includes applicable constraint summaries for the component
- [x] `test_eligible_suppliers_shows_max_qty_per_supplier` — when concentration limit applies, shows max quantity each supplier can receive
- [x] `test_eligible_suppliers_air_freight_annotation` — when air freight is active, shows both standard and air freight delivery dates for international suppliers
- [x] Run tests — verify RED

#### Implementation (GREEN)
- [x] Add `_annotate_supplier(ctx, component_id, supplier_row, gap_row) -> str` helper that builds the annotation block for one supplier:
...
- [x] Add `_constraint_notes(ctx, component_id, gap_row) -> str` that summarizes applicable constraints:
  - Concentration limit with max qty per supplier
  - Critical component status
  - Hazmat flag
  - Cert requirements
- [x] Rewrite `get_eligible_suppliers` to:
...
- [x] In `prompts.py`, update Decision Guidelines:
...
- [x] Remove or simplify workflow step about `check_concentration` (now less needed since `place_order` enforces and `get_eligible_suppliers` shows it)
...
- [x] All `tests/test_tools.py` tests pass
- [ ] All `tests/test_verification.py` tests pass (including previously-failing F1, F2)
- [x] `uv run ruff check .` clean (for changed files)
- [x] `uv run ruff format .` clean (for changed files)
- [ ] 6 scenarios run end-to-end without crashing (requires agent re-run)
...
**Status:** Code Complete — Steps 1-5 + 7 done. Step 6 (integration re-run) pending.
